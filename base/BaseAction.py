...
class WebDriver(object):
    def __init__(self):
        self.timeout = int(read_ini.get_value("sys_default", "timeout"))
        self.broswer = read_ini.get_value("sys_default", "broswer")
        if self.broswer:
            self.driver = open_browser(self.broswer)
        else:
    ...
    def write_cookie(self, file_name=None):
        """
        将cookie信息写入json文件中
        :param file_name:
        :return:
        ...
        if file_name:
            ...
            file_path = os.path.join(base_path + '/config/' + 'cookie.json')
        logging.info("需要写入cookie的json文件路径为:{0}".format(file_path))
        dict_list = {}
        try:
            for i in self.get_cookie():
                dict_list[i['name']] = i['value']
            data = json.dumps(dict_list)
            logging.info("cookie内容为：".format(data))
            with open(file_path, 'w', encoding="UTF-8") as f:
                f.write(data)
        except:
            logging.error("写入cookie失败")
        Logger().close_logger()
        return None
    ...
    def custom_add_cookie(self, file_name=None):
        cookie_list = {}
        base_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if file_name:
            file_path = os.path.join(base_path + '/config/' + file_name)
            ...
            file_path = os.path.join(base_path + '/config/' + 'cookie.json')
        try:
            logging.info("读取json文件路径：{0}".format(file_path))
            ...
            logging.error("获取{0}文件的json对象失败".format(file_path))
        Logger().close_logger()

    def del_cookie(self, name):
        """
        删除cookie信息，传入cookie名称即可
        :param name: 要删除的cookie信息
        :return:
        """
    ...
    def get_window_size(self):
        ...
        """
        logging.info(
            "窗口宽为:{0} 高为:{1}".format(self.driver.get_window_size()['width'], self.driver.get_window_size()['height']))
